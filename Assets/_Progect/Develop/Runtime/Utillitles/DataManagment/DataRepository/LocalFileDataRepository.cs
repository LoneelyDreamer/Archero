...
namespace Assets._Progect.Develop.Runtime.Utillitles.DataManagment.DataRepository
{
    public class LocalFileDataRepository : IDataRepository
    {
        ...
        private readonly string _saveFileExtansion;

        public LocalFileDataRepository(string folderPath, string saveFileExtansion)
        {
            _folderPath = folderPath;
            _saveFileExtansion = saveFileExtansion;
        }
        ...
        private string FullPathFor(string key)
            => Path.Combine(_folderPath, key) + "." + _saveFileExtansion;
